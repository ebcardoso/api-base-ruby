...
RSpec.describe Api::V1::UsersController, type: :request do
  context 'Sucess Path' do
    before(:all) do
      ...
      delete api_v1_user_path(@current_user.id.to_s), headers: headers
    ...
    it 'Should have a message' do
      ...
      expect(@response&.dig('message')).to eq(I18n.t('user.destroy.success'))
    end
  end
  ...
  context 'Failure - User not Found' do
    before(:all) do
      ...
      delete api_v1_user_path('1235'), headers: headers
      @response = JSON.parse(response.body)
    end
